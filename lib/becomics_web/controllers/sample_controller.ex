defmodule BecomicsWeb.SampleController do
  ...
  defp select_loop(comics, [sample | t], count_down, overlap), do: select_loop(Enum.drop(comics, sample), t, count_down - 1, overlap)
	...
	List.duplicate(high_per_day, days_with_high_selection) ++ List.duplicate(low_per_day, total_selection - days_with_high_selection)
    end

  if Mix.env() === :test do
  def test_select(comics, total_selection, which), do: select(comics, total_selection, which, 0)
  def test_select(comics, total_selection, which, overlap), do: select(comics, total_selection, which, overlap)
